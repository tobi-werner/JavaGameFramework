...
public class BoardTest {
	...
	public final void testGetTileAtIndex() {
		...
		assertNull(board.getTileAtIndex(new Vector2f(4, 2)));
		assertNull(board.getTileAtIndex(new Vector2f(2, 4)));
	...
	public final void testSetTileAtIndex() {
		...
		assertFalse(board.setTileAtIndex(new Vector2f(4, 2), tile));
		assertFalse(board.setTileAtIndex(new Vector2f(2, 4), tile));
	...
	public final void testTransformScreenCoordinateToIndex() {
		...
		// move the board two tiles to the side
		board.setPosition(new Vector2f(2*Constants.GamePlay.tileSize.x, 0));
		
		Vector2f index3 = board.transformScreenCoordinateToIndex(Constants.Base.zeroVector);
		assertEquals(new Vector2f(-2, 0), index3);
		
		Vector2f index4 = board.transformScreenCoordinateToIndex(new Vector2f(Constants.GamePlay.tileSize.x+10, Constants.GamePlay.tileSize.y+10));
		assertEquals(new Vector2f(-1,1), index4);
	}
	...
	public final void testTransformIndexToScreenCoordinate() {
		...
		Vector2f position4 = board.transformScreenCoordinateToIndex(Constants.Base.zeroVector);
		assertEquals(new Vector2f(-2,0), position4);
		
		Vector2f position5 = board.transformScreenCoordinateToIndex(new Vector2f(Constants.GamePlay.tileSize.x-10, Constants.GamePlay.tileSize.y-10));
		assertEquals(new Vector2f(-1,1), position5);
	...
	public final void testValidTileAtIndex() {
		...
		// Tile already exists at index
		assertFalse(board.validTileAtIndex(new Vector2f(1,1)));
		
		// Es gibt kein benachbartes Tile
		assertFalse(board.validTileAtIndex(new Vector2f(3,3)));
		assertFalse(board.validTileAtIndex(new Vector2f(0,0)));
